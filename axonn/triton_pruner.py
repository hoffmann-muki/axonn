...
_ERROR_DTYPE_MAP: dict = {
    ...
    "fp8e4": (torch.int8, tl.float8e4b15, True),
    "fp8e5": (torch.int8, tl.float8e5, True),
}
...
class TritonGradientPruner:
    ...
    @torch.no_grad()
    def prune(self, tensor: torch.Tensor, key=0, timer=None, return_timing: bool = False):
        """
        ...
        Args:
            ...
            timer: optional timer object with start()/stop()
            return_timing: if True, also return CUDA event timings

        Returns:
            Pruned tensor, or (tensor, timing) when return_timing is True
        """
